function [hstruct] = pmf_TraceGain3(fignum, SrcStruct1, SrcStruct2, series1, series2, sweep)
...
for l = 1:3
    mm = 1;
    for m = sweep
        SrcWave = SrcStruct1(series1).Voltage{m,1}((5e4+8e4*(l-1)+1):(7e4+8e4*(l-1)));
        plot(haxes(l), xwave, SrcWave, 'k', 'LineWidth', 0.3)
        SrcWave = SrcStruct2(series2).Voltage{m,1}((5e4+8e4*(l-1)+1):(7e4+8e4*(l-1)));
...
% Create textbox
annotation(figure1,'textbox',...
    [0.699881226222763 0.365975572725125 0.200680272108843 0.0573449087746946],...
    'String','-800 µA',...
    'HorizontalAlignment','center',...
    'FontSize',12,...
    'FontName','Arial',...
...
% Create textbox
annotation(figure1,'textbox',...
    [0.415933422821441 0.366900345186609 0.200113378684808 0.0554953638517275],...
    'String','+800 µA',...
    'HorizontalAlignment','center',...
    'FontSize',12,...
    'FontName','Arial',...
